<template>
    <nav class="w-full fixed top-0 left-0 bg-gray-900/80 backdrop-blur-md text-white py-4 shadow-md z-50">
      <div class="container mx-auto flex justify-between items-center px-6">
        <!-- Logo -->
        <router-link to="/" class="text-2xl font-bold text-white hover:text-green-400 transition">
          Raphter.io
        </router-link>
        ...
        <!-- Bouton Connexion -->
        <button class="hidden md:flex items-center gap-2 bg-green-500 hover:bg-green-600 px-4 py-2 rounded-full text-white font-semibold shadow-md transition">
      ...
      <transition name="slide">
        <div v-if="isMenuOpen" class="md:hidden absolute top-full left-0 w-full bg-gray-900/90 backdrop-blur-md p-6 text-center shadow-md">
          <ul class="space-y-4">
